...
describe("team-mode tool registry wiring", () => {
  test("passes ctx.client into every team tool factory", () => {
    ...
    expect(createTeamSendMessageTool).toHaveBeenCalledWith(expect.anything(), client, undefined, expect.any(Function))
    ...
    expect(createTeamStatusTool).toHaveBeenCalledWith(expect.anything(), client, expect.anything(), undefined, expect.any(Function))
